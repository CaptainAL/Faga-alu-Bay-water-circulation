...
for shape in grid.shapes():
    ...
    if len(gridpoints) >= 4:
        ...
        cMap,cNorm = mpl.cm.rainbow, mpl.colors.Normalize(vmin=0,vmax=.4) ## =Num of observations
...
plt.savefig(figdir+'Paxes gridded/'+'drifters P axes with arrows-colored by speed-'+by_dep+'.svg',transparent=True)
