...
## Wave data
document.add_paragraph("Incident wave conditions were recorded by a NIWA Dobie-A wave/tide gauge (DOBIE) deployed on the southern reef slope at a depth of 10 m (Figure "+Study_Area_map['fig_num']+"). The DOBIE sampled a 512s burst at 2 Hz every hour. The DOBIE malfunctioned and recorded no data coinciding with the ADCP deployment, but compared well (not shown) with NOAA WaveWatchIII (WW3; REFERENCE) modeled data on swell height and direction for the recorded data (Hoeke et al., 2011). WW3 model data, calibrated to DOBIE wave data, were used to define forcing during the ADCP and drifter deployments.")
## Wind data
document.add_paragraph("Wind speed, wind direction, barometric pressure, and precipitation were recorded at 15 min intervals using a Davis VantagePro weather station installed near the stream mouth, approximately 5 m above sea level on a pole mounted to a building (Figure "+Study_Area_map['fig_num']+"). Meteorological and tide data were also recorded at NOAA's National Data Buoy Center's NSTP6 station (REFERENCE) located approximately 1.8 km north of Faga'alu. Wind speed, wind direction, barometric pressure, and tidal elevation data were also recorded at NSTP6 at 6 min intervals. For this study, wind conditions are sufficiently described qualitatively so the topographic effects on wind speed and direction recorded at the stations are assumed to be inconsequential.")


## Analytical Methods
document.add_heading('Analytical Methods',level=3)

document.add_paragraph("Data from the drifters and ADCP's was subset by end-member forcing condition, and two techniques were used to compare the drifter results with the ADCP results: progressive vectors of cumulative flow assuming spatial homogeneity and Empirical orthogonal functions (EOF).")

## Progressive Vectors
document.add_paragraph("A series of 1 h progressive vector diagrams of cumulative flow were computed from ADCP data following the methodology used by Storlazzi et al. (2006). The progressive vectors were compared to simultaneous drifter tracks to demonstrate the usefulness of drifters for illustrating spatially variable flows compared to projected flow from fixed ADCP's. The progressive vectors from the ADCP's illustrate the temporal flow variability at those fixed points during the 1 hr drift duration, but progressive vectors are calculated assuming spatial homogeneity of the flow, causing them to move onshore in some instances.")

## EOF's, mean current, residence time
document.add_paragraph("EOF principal axes, variance ellipses, and mean flow vectors were calculated from ADCP data and spatially binned drifter data (100 m x 100 m) (MacMahan et al., 2010) under differnt endmember forcing conditions. Where drifters did not travel through a spatial bin, no EOF or residence time could be calculated. EOF's and mean flow vectors from drifters and ADCP's were compared to determine if the short-term observations from the drifters were similar to the long-term ADCP, and to demonstrate the usefulness of Lagrangian methods for descirbing spatial flow patterns compared to fixed Eulerian methods.") 
document.add_paragraph("Mean flow vectors were also used to calculate water residence time over the reef under different forcing conditions.")
