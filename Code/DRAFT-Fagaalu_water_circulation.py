...
document.add_paragraph("A series of 1 h progressive vector diagrams of projected cumulative flow were computed from ADCP data (Storlazzi et al, 2006). Progressive vectors are calculated assuming spatial homogeneity of the flow, causing them to move onshore in some instances. The progressive vectors from ADCP data mainly illustrate how variable the flow velocity is at a fixed point over time: a progressive vecotr moving in a straight line indicates flow direction at the instrument does not change over time. The progressive vectors for AS1 and AS2 show little variation in flow direction, indicating the flow velocity is consistent (Figure "+Progressive_Vectors_ADCP_vs_Drifters['fig_num']+"). The progressive vectors for AS3 are much more erratic, and travel relatively shorter distances due to the lower flow speeds observed over the northern reef.")
